...
class WorkoutPlanRepositoryImpl @Inject constructor(private val api: WorkoutPlanApi) : WorkoutPlanRepository {
    ...
    override suspend fun update(id: Long, request: UpdateWorkoutPlanRequest) = networkCall { api.update(id, RemoteUpdateWorkoutPlanRequest(request.startDate, request.endDate, request.status)).data?.toDomain() ?: throw AppException.InvalidResponse("Missing workout plan") }
    override suspend fun addDay(planId: Long, request: WorkoutPlanDayRequest) = networkCall { api.addDay(planId, DayRequest(request.dayOfWeek, request.exercises.map { ExerciseRequest(it.sourceTemplateExerciseId, it.exerciseId, it.orderIndex, it.sets, it.repetitions, it.restSeconds) })).data?.toDomain() ?: throw AppException.InvalidResponse("Missing workout plan") }
    override suspend fun addExercise(planId: Long, day: Int, request: WorkoutPlanExerciseRequest) = networkCall { api.addExercise(planId, day, ExerciseRequest(request.sourceTemplateExerciseId, request.exerciseId, request.orderIndex, request.sets, request.repetitions, request.restSeconds)).data?.toDomain() ?: throw AppException.InvalidResponse("Missing workout plan") }
    override suspend fun updateExercise(id: Long, request: WorkoutPlanExerciseRequest) = networkCall { api.updateExercise(id, ExerciseRequest(request.sourceTemplateExerciseId, request.exerciseId, request.orderIndex, request.sets, request.repetitions, request.restSeconds)).data?.toDomain() ?: throw AppException.InvalidResponse("Missing workout plan") }
